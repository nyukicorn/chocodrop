...
const pages = {
  ...
  guide: () => `${heading('HOW TO DROP', 'ひとつ置いたら、はじまる。', 'ブラウザと、好きな素材。それだけで試せます。')}<div class="guide-layout"><div><section class="guide-step"><span class="step-number">01</span><div><h2>はじまりの世界を開く</h2><p>まずはサンプルを配置してみましょう。ツールの接続や生成サービスの設定は不要です。</p><button class="button primary" data-demo>デモを開く　→</button></div></section><section class="guide-step"><span class="step-number">02</span><div><h2>手元のファイルを読み込む</h2><p>デモ右下の🍫を開き、Importからファイルを選びます。まずは画像1枚で試すとわかりやすくなります。</p></div></section><section class="guide-step"><span class="step-number">03</span><div><h2>景色を、いろんな角度から</h2><p>ドラッグやスクロールで視点を動かし、置いた素材を眺めてみましょう。操作方法は各デモの案内をご覧ください。</p></div></section><h2>気になること</h2><details><summary>自分の3Dサイトにも組み込めますか？</summary><p>現在の組み込み実装はThree.js向けです。WebGLを使っているすべてのサイトに、そのまま対応するわけではありません。ほかの3DエンジンやWebGPUでの動作は未確認です。</p></details><details><summary>トップの遊園地では何を置けますか？</summary><p>PNG・JPEG・WebPの画像、MP4・WebMの動画、外部ファイルを参照しないGLB 2.0の3Dモデルを、1ファイル32MiBまで選べます。素材は1つずつ置き換わり、動画は無音で繰り返し再生します。圧縮拡張を必要とするGLBなど、一部のファイルは読み込めません。</p></details><details><summary>素材をAIで生成する必要はありますか？</summary><p>ありません。写真や作成済みのファイルを読み込めます。生成した素材も、ファイルとして用意すれば同じように使えます。</p></details><details><summary>置いたものは保存されますか？</summary><p>この案内では保存・再開を保証していません。ページを閉じたり再読み込みする前に、必要な記録を残してください。</p></details><details><summary>Codex・Claude Code・Gemini CLIから使うには？</summary><p>ローカルのMCPを設定すると、会話から素材の配置を依頼できます。<a href="#connect">接続の手順を見る →</a></p></details><details><summary>ブックマークレットは使えますか？</summary><p>使えます。ローカルdaemonを起動し、<a href="examples/bookmarklet-v2.html">登録ページ</a>からブックマークへ追加します。対象ページがThree.jsシーンを公開していることなど、利用条件があります。</p></details></div><aside class="side-note"><span class="eyebrow">A LITTLE NOTE</span><h3>最初は、小さな画像から。</h3><p>画像・動画・3Dモデルの読み込みに対応しています。形式やサイズの制限は利用方法によって異なります。</p><p>このサイトのデモはThree.jsとWebGLを使用しています。WebGLが利用できるブラウザが必要です。音声やXRなどの追加機能は、端末や権限に依存します。</p><a class="text-link" href="#connect">ツールから配置する　→</a></aside></div>`,
  connect: () => `${heading('CONNECT YOUR TOOLS', 'いつもの相棒と、世界をつくる。', 'Codex・Claude Code・Gemini CLIをはじめ、いつものCLIやMCPを制作に活かす。つくった画像・動画・3Dモデルを、ChocoDropの世界に置いてみましょう。')}<section class="status-grid"><div class="status-card"><span class="badge">ファイルでつなぐ</span><h2>好きなツールでつくって、置く</h2><p>普段使う制作ツールやCLI、接続済みの生成MCPで素材を作成 → 対応形式のファイルとして保存 → ChocoDropのImportで選択。この方法なら、ChocoDropへのMCP接続は不要です。</p><p>生成サービスの設定・契約は各ツール側で行います。出力されたファイルがChocoDropの対応形式・サイズに合うことを確認してください。</p></div><div class="status-card"><span class="badge">MCPでつなぐ</span><h2>会話から、素材を配置する</h2><p>Codex・Claude Code・Gemini CLIにChocoDropのMCPを登録すると、素材フォルダ内のファイルを会話から配置できます。以下に3つのツールの設定例を用意しました。</p><p>ChocoDropのMCPは素材を生成せず、許可したローカルファイルを配置します。生成には普段お使いのツールを利用してください。</p></div></section><section class="section"><h2>たとえば、こんな頼み方。</h2><p>「接続済みの制作ツールで花の画像を作り、素材フォルダにPNGで保存して。その画像をChocoDropに置いて」</p><p class="subtle-note">生成ツールとChocoDropのMCPをそれぞれ設定した環境での依頼例です。ChocoDrop自体に画像・動画の生成機能が含まれる、という意味ではありません。</p></section><div class="setup"><aside class="setup-nav" role="tablist" aria-label="接続するツール"><button class="setup-tab" id="tab-codex" role="tab" aria-selected="true" aria-controls="setup-content" data-tool="codex">Codex　→</button><button class="setup-tab" id="tab-claude" role="tab" aria-selected="false" aria-controls="setup-content" tabindex="-1" data-tool="claude">Claude Code　→</button><button class="setup-tab" id="tab-gemini" role="tab" aria-selected="false" aria-controls="setup-content" tabindex="-1" data-tool="gemini">Gemini CLI　→</button></aside><section class="setup-panel" id="setup-content" role="tabpanel" aria-labelledby="tab-codex"></section></div><p class="subtle-note">既存のThree.jsページへブックマークからUIを加える場合は、<a href="examples/bookmarklet-v2.html">ブックマークレット登録ページ</a>をご利用ください。</p>`,
  about: () => `${heading('PROJECT NOTES', 'いま使えること、これからのこと。', '見た目と、実際の機能の状態を分けてお伝えします。')}<div class="status-grid"><section class="status-card"><span class="badge">公開中</span><h2>ブラウザのデモ</h2><p>遊園地で画像・動画・GLBを置き、公開中の7つの世界へ移動できます。Three.jsとWebGLを使用します。</p><a href="#worlds" class="text-link">世界を選ぶ →</a></section><section class="status-card"><span class="badge">ローカルで利用</span><h2>Codex・Claude Code・Gemini CLI連携</h2><p>公開されているコードを自分のPCで動かすMCPです。GitHub Pages上でMCPサーバーが動いているわけではありません。</p><a href="#connect" class="text-link">接続方法を見る →</a></section><section class="status-card"><span class="badge">Chromeで検証</span><h2>ブックマークレット</h2><p>ローカルdaemonを起動し、Three.jsシーンを公開しているページで使用できます。ページのセキュリティ設定やシーンの構成によっては利用できません。</p><a href="examples/bookmarklet-v2.html" class="text-link">登録ページを開く →</a></section><section class="status-card"><span class="badge">別途検証中</span><h2>XR・Quest</h2><p>ブラウザ版とは別の動作条件があります。Quest実機、音声生成、ストア配布を含むXR機能は、この公開導線の利用可能範囲には含めていません。</p></section></div>`
};
const snippets = {
  ...
  gemini: 'gemini mcp add --scope user chocodrop node /absolute/path/to/chocodrop/scripts/chocodrop-mcp.mjs -- --assets-dir /absolute/path/to/your/assets\ngemini mcp list'
};
...
function setup(tool = 'codex') {
  const names = { codex: 'Codex', claude: 'Claude Code', gemini: 'Gemini CLI' };
