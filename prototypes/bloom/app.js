...
const worlds = [
  { id: 'basic', name: 'はじまりの世界', text: 'ひとつ置くところから。好きな画像や3Dモデルで、最初の一歩を。', tag: 'はじめての方へ', path: 'basic', image: 'basic.png' },
  { id: 'toy', name: 'ちいさな遊園地', text: '観覧車とメリーゴーランド。昼と夜で変わる、おもちゃの街へ。', tag: '昼も、夜も', path: 'toy-city', image: 'toy-dark.png' },
  { id: 'garden', name: '音楽の花園', text: '音が鳴ると、花が広がり、光の粒が舞う。サンプル音で体験してみましょう。', tag: '花のある世界', path: 'music-garden', image: 'garden.png' },
  {id:'ocean',name:'海底世界',text:'光と生き物が漂う、ピクセルの海へ。',tag:'水の世界',path:'pixel-ocean',image:'ocean.png'},
  {id:'space',name:'宇宙空間',text:'星の間を旅する、広い空間へ。',tag:'星の世界',path:'space',image:'space.png'},
  {id:'wabi',name:'侘び寂び',text:'静かな余白を味わう世界へ。',tag:'静けさ',path:'wabi-sabi',image:'wabi.png'},
  {id:'lofi',name:'Lo-fi Room',text:'落ち着いた部屋で、自分の時間を。',tag:'部屋',path:'lofi-room',image:'lofi.png'}
];
...
const pages = {
  home: () => `<section class="hero"><div class="hero-copy"><span class="eyebrow">SMALL DROPS, BIG WORLDS</span><h1>ちょこっとDrop。<br>世界が<em>咲く。</em><span class="petal" aria-hidden="true"></span></h1><p>好きな素材を置く。<br>あなただけの世界が、花ひらく。</p><div class="actions"><button class="button primary" data-demo>✿　ひとつ、置いてみる　→</button><a class="button" href="#guide">使い方を見る</a></div><p class="caption">ChocoDropは、写真・動画・3Dモデルを<br>ブラウザの3D空間に置いて楽しめるツールです。</p><p class="caption">Three.js対応 · この遊園地で、手元の素材を試せます。</p></div><div class="live-world"><iframe id="park-preview" src="scene.html?v=import7&theme=${document.documentElement.dataset.theme}" title="動く遊園地。ドラッグで視点を動かせます"></iframe><div class="park-actions"><button class="button" data-park-zoom="in" aria-label="遊園地を拡大">＋ 拡大</button><button class="button" data-park-zoom="out" aria-label="遊園地を縮小">− 縮小</button><button class="button" data-park-expand>大きく見る</button><a class="button" href="compare.html" target="_blank" rel="noopener">元の遊園地と比較 ↗</a><button class="button" data-park-theme="light">☀ 昼</button><button class="button" data-park-theme="dark">☾ 夜</button><button class="button primary" data-park-sample>猫ちゃんを置く ✿</button><label class="button" for="park-file">画像・動画・GLBを選ぶ<input id="park-file" type="file" accept="image/png,image/jpeg,image/webp,video/mp4,video/webm,.glb" hidden></label><button class="button" data-park-pause aria-pressed="false">動きを止める</button></div><p class="caption">実際に動く3D空間です。ドラッグで見回す · スクロールはページ移動 · 拡大・縮小はボタンで · ファイルは32MBまで</p></div></section><section class="section"><div class="section-heading"><div><span class="eyebrow">FIND YOUR WORLD</span><h2>次は、どんな世界に？</h2></div><a class="text-link" href="#worlds">世界を選ぶ　→</a></div><div class="world-grid">${cards(worlds.filter(w=>['garden','ocean','space'].includes(w.id)))}</div><button class="button" data-garden>花園を動くプレビューで見る →</button></section><section class="connect-band"><div><h2>開発の相棒と、もっと楽しく。</h2><p>CLIやMCPでつくった素材を、あなたの世界にドロップ。</p></div><div class="tool-names"><span>Codex</span><span>Claude Code</span></div><a class="button" href="#connect">ツール連携について　→</a></section><section class="section"><div class="section-heading"><h2>小さなひとつから、はじめよう。</h2></div><div class="steps"><div class="step"><span class="step-number">01</span><h3>世界を選ぶ</h3><p>気になる景色を開いて、まずは眺めてみる。</p></div><div class="step"><span class="step-number">02</span><h3>好きな素材を置く</h3><p>遊園地の右下「🍫 素材を選ぶ」か、下の「画像・動画・GLBを選ぶ」から。</p></div><div class="step"><span class="step-number">03</span><h3>自分の景色を見つける</h3><p>視点を動かしながら、あなたらしい一枚を。</p></div></div></section>`,
  worlds: () => `${heading('EXPLORE', '好きな世界へ、ひとっ飛び。', '気になる景色を選んで、ブラウザの中へ。公開されている7つの世界から選べます。画像は実際のデモ画面です。')}<div class="filters" aria-label="世界の絞り込み"><button class="filter" data-filter="all" aria-pressed="true">すべて</button><button class="filter" data-filter="basic" aria-pressed="false">はじめて</button><button class="filter" data-filter="toy" aria-pressed="false">昼と夜</button><button class="filter" data-filter="garden" aria-pressed="false">花のある世界</button></div><div class="world-grid">${cards()}</div><button class="button" data-garden>花園を動くプレビューで見る →</button><p class="subtle-note">各デモは別タブで開きます。このサイトのDark / Lightと、デモの昼夜設定は別々です。遊園地ではデモ内のボタンで切り替えられます。</p><section class="connect-band"><div><h2>まずは、ひとつ置いてみる？</h2><p>サンプルからなら、素材の準備もいりません。</p></div><button class="button primary" data-demo>はじまりの世界を開く　→</button></section>`,
  guide: () => `${heading('HOW TO DROP', 'ひとつ置いたら、はじまる。', 'ブラウザと、好きな素材。それだけで試せます。')}<div class="guide-layout"><div><section class="guide-step"><span class="step-number">01</span><div><h2>はじまりの世界を開く</h2><p>まずはサンプルを配置してみましょう。ツールの接続や生成サービスの設定は不要です。</p><button class="button primary" data-demo>デモを開く　→</button></div></section><section class="guide-step"><span class="step-number">02</span><div><h2>手元のファイルを読み込む</h2><p>デモ右下の🍫を開き、Importからファイルを選びます。まずは画像1枚で試すとわかりやすくなります。</p></div></section><section class="guide-step"><span class="step-number">03</span><div><h2>景色を、いろんな角度から</h2><p>ドラッグやスクロールで視点を動かし、置いた素材を眺めてみましょう。操作方法は各デモの案内をご覧ください。</p></div></section><h2>気になること</h2><details><summary>自分の3Dサイトにも組み込めますか？</summary><p>現在の組み込み実装はThree.js向けです。WebGLを使っているすべてのサイトに、そのまま対応するわけではありません。ほかの3DエンジンやWebGPUでの動作は未確認です。</p></details><details><summary>トップの遊園地では何を置けますか？</summary><p>PNG・JPEG・WebPの画像、MP4・WebMの動画、外部ファイルを参照しないGLB 2.0の3Dモデルを、1ファイル32MiBまで選べます。素材は1つずつ置き換わり、動画は無音で繰り返し再生します。圧縮拡張を必要とするGLBなど、一部のファイルは読み込めません。</p></details><details><summary>素材をAIで生成する必要はありますか？</summary><p>ありません。写真や作成済みのファイルを読み込めます。生成した素材も、ファイルとして用意すれば同じように使えます。</p></details><details><summary>置いたものは保存されますか？</summary><p>この案内では保存・再開を保証していません。ページを閉じたり再読み込みする前に、必要な記録を残してください。</p></details><details><summary>CodexやClaude Codeから使うには？</summary><p>ローカルのMCPを設定すると、会話から素材の配置を依頼できます。<a href="#connect">接続の手順を見る →</a></p></details><details><summary>ブックマークレットは使えますか？</summary><p>現在は見直し中です。配布パッケージの不足があるため、この試作では導入ボタンを設けていません。<a href="#about">対応状況を見る →</a></p></details></div><aside class="side-note"><span class="eyebrow">A LITTLE NOTE</span><h3>最初は、小さな画像から。</h3><p>画像・動画・3Dモデルの読み込みに対応しています。形式やサイズの制限は利用方法によって異なります。</p><p>このサイトのデモはThree.jsとWebGLを使用しています。WebGLが利用できるブラウザが必要です。音声やXRなどの追加機能は、端末や権限に依存します。</p><a class="text-link" href="#connect">ツールから配置する　→</a></aside></div>`,
  connect: () => `${heading('CONNECT YOUR TOOLS', 'いつもの相棒と、世界をつくる。', 'Codex・Claude Codeをはじめ、いつものCLIやMCPを制作に活かす。つくった画像・動画・3Dモデルを、ChocoDropの世界に置いてみましょう。')}<section class="status-grid"><div class="status-card"><span class="badge">ファイルでつなぐ</span><h2>好きなツールでつくって、置く</h2><p>普段使う制作ツールやCLI、接続済みの生成MCPで素材を作成 → 対応形式のファイルとして保存 → ChocoDropのImportで選択。この方法なら、ChocoDropへのMCP接続は不要です。</p><p>生成サービスの設定・契約は各ツール側で行います。出力されたファイルがChocoDropの対応形式・サイズに合うことを確認してください。</p></div><div class="status-card"><span class="badge">MCPでつなぐ</span><h2>会話から、素材を配置する</h2><p>Codex・Claude CodeにChocoDropのMCPを登録すると、素材フォルダ内のファイルを会話から配置できます。以下に両ツールの設定例を用意しました。</p><p>ほかのMCPクライアントは、ローカルのstdioサーバーを起動できることが条件です。ツールごとの接続検証は別途必要です。</p></div></section><section class="section"><h2>たとえば、こんな頼み方。</h2><p>「接続済みの制作ツールで花の画像を作り、素材フォルダにPNGで保存して。その画像をChocoDropに置いて」</p><p class="subtle-note">生成ツールとChocoDropのMCPをそれぞれ設定した環境での依頼例です。ChocoDrop自体に画像・動画の生成機能が含まれる、という意味ではありません。</p></section><div class="setup"><aside class="setup-nav" role="tablist" aria-label="接続するツール"><button class="setup-tab" id="tab-codex" role="tab" aria-selected="true" aria-controls="setup-content" data-tool="codex">Codex　→</button><button class="setup-tab" id="tab-claude" role="tab" aria-selected="false" aria-controls="setup-content" tabindex="-1" data-tool="claude">Claude Code　→</button></aside><section class="setup-panel" id="setup-content" role="tabpanel" aria-labelledby="tab-codex"></section></div>`,
  about: () => `${heading('PROJECT NOTES', 'いま使えること、これからのこと。', '見た目の試作と、実際の機能の状態を分けてお伝えします。')}<div class="status-grid"><section class="status-card"><span class="badge">公開中</span><h2>ブラウザのデモ</h2><p>はじまりの世界・遊園地・花園へ、この試作から移動できます。画像は実際の公開ページを撮影しています。音声・XR・全端末での動作は未確認です。</p><a href="#worlds" class="text-link">世界を選ぶ →</a></section><section class="status-card"><span class="badge">ローカルで利用</span><h2>Codex・Claude Code連携</h2><p>公開されているコードを自分のPCで動かすMCPです。GitHub Pages上でMCPサーバーが動いているわけではありません。</p><a href="#connect" class="text-link">接続方法を見る →</a></section><section class="status-card"><span class="badge">見直し中</span><h2>ブックマークレット</h2><p>現行の配布パッケージにSDKとUIの不足が見つかっています。配布内容の修正と、対象サイトでの動作確認を終えてから導入の案内を戻します。</p><p>この試作では修理や再公開は行っていません。</p></section><section class="status-card"><span class="badge">今回の試作</span><h2>サイト全体の新しい装い</h2><p>紫がかったダークと、クリーム色のライト。ページを移動しても、選んだ色合いが続きます。トップには動く遊園地と素材の配置体験があります。</p><p>公開サイトと各デモの画面デザインは、まだ置き換えていません。</p></section></div>`
};
const snippets = {
  prep: 'git clone https://github.com/nyukicorn/chocodrop.git\ncd chocodrop\nnpm ci\nnpm run build',
  codex: 'codex mcp add chocodrop -- node /absolute/path/to/chocodrop/scripts/chocodrop-mcp.mjs --assets-dir /absolute/path/to/your/assets\ncodex mcp get chocodrop',
  claude: 'claude mcp add --transport stdio --scope local chocodrop -- node /absolute/path/to/chocodrop/scripts/chocodrop-mcp.mjs --assets-dir /absolute/path/to/your/assets\nclaude mcp get chocodrop'
};
...
function setup(tool = 'codex') {
  ...
  panel.innerHTML = `<span class="eyebrow">LOCAL MCP</span><h2>${name}とつなぐ</h2><p>Node.js・npmと${name}のCLIが使える環境で設定します。素材専用のフォルダを用意してください。</p><div class="setup-flow"><h3>1. ChocoDropを準備する</h3>${codeBlock('prep', 'ターミナル')}<h3>2. MCPを登録する</h3><p><code>/absolute/path/to/…</code>は、ご自身のPCの絶対パスに置き換えます。空白を含むパスは引用符で囲んでください。</p>${codeBlock(tool, name)}<h3>3. 会話から配置する</h3><p>接続後に「ChocoDropのget_statusでURLを教えて」と依頼し、返されたローカルURLを開きます。その後「素材フォルダの画像をimport_assetで配置して」と依頼します。</p><p class="subtle-note">PNG・JPEG・WebP・MP4・WebM・自己完結したGLBに対応。1ファイル32MiBまで。接続先のブラウザを開いた状態で使ってください。</p>${external('getting-started.html', '詳しい現行ガイドを見る')}</div>`;
}
function syncTheme() {
...
document.addEventListener('click', async event => {
  ...
  if (button.dataset.filter) {
    ...
    document.querySelectorAll('[data-world]').forEach(card => { card.hidden = button.dataset.filter !== 'all' && card.dataset.world !== button.dataset.filter; });
  }
...
document.addEventListener('keydown', event => {
  ...
  const tool = event.key === 'Home' ? 'codex' : event.key === 'End' ? 'claude' : event.target.dataset.tool === 'codex' ? 'claude' : 'codex';
  setup(tool);
