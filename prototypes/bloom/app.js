...
const snippets = {
  codex: 'npx --yes @chocodrop/setup@alpha --client codex',
  claude: 'npx --yes @chocodrop/setup@alpha --client claude',
  gemini: 'npx --yes @chocodrop/setup@alpha --client gemini'
};
...
function setup(tool = 'codex') {
  ...
  panel.innerHTML = `<span class="eyebrow">LOCAL MCP</span><h2>${name}とつなぐ</h2><p>Node.js・npmと${name}のCLIが使える環境で、次の1コマンドを実行します。GitHubのcloneは不要です。</p><div class="setup-flow"><h3>1. MCPを登録する</h3>${codeBlock(tool, name)}<p>内容を表示して確認した後、<code>~/ChocoDropAssets</code>を作成し、${name}のユーザー設定へ登録します。</p><h3>2. CLIを再起動する</h3><p>再起動後に「ChocoDropのget_statusでURLを教えて」と依頼し、返されたローカルURLを開きます。</p><h3>3. つくって、置く</h3><p>普段の生成MCPやCLIで素材フォルダへファイルを保存し、「ChocoDropのimport_assetで配置して」と依頼します。</p><p class="subtle-note">PNG・JPEG・WebP・MP4・WebM・自己完結したGLBに対応。1ファイル32MiBまで。接続先のブラウザを開いた状態で使ってください。</p>${external('getting-started.html', '詳しい現行ガイドを見る')}</div>`;
}
function syncTheme() {
