...
test('builds client-specific registration commands around the published MCP package', () => {
  ...
  const antigravity = registration('antigravity', assets, '@chocodrop/mcp@0.1.0-alpha.1', 'pnpm', '/Users/example');

  assert.deepEqual(codex.add, [
    'mcp', 'add', 'chocodrop', '--', 'pnpm', 'dlx', '@chocodrop/mcp@0.1.0-alpha.1', '--assets-dir', assets,
  ]);
  ...
  assert.deepEqual(registration('codex', assets, '@chocodrop/mcp@0.1.0-alpha.1', 'npx').add.slice(4, 7), [
...
test('preserves Antigravity config while adding ChocoDrop', async () => {
  ...
  const item = registration('antigravity', path.join(root, 'assets'), '@chocodrop/mcp@0.1.0-alpha.1', 'pnpm', root);
  try {
