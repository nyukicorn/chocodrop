...
```bash
# Codexだけに登録
pnpm dlx @chocodrop/setup@alpha --client codex --yes

# 素材フォルダを指定
pnpm dlx @chocodrop/setup@alpha --assets-dir /path/to/assets

# 変更内容だけ確認
pnpm dlx @chocodrop/setup@alpha --dry-run
```

pnpmがない環境では`npx --yes @chocodrop/setup@alpha`も使用できます。
